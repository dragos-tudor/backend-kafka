...
{
  internal static DateTime CalculateNextAttemptAt(int retryCount, DateTime date, RetryMessageOptions options)
  {
    var retryInterval = CalculateNextRetryInterval(retryCount, options);
    return date.Add(retryInterval);
  }

  internal static TimeSpan CalculateNextRetryInterval(int retryCount, RetryMessageOptions options)
  {
