...
partial class OutboxFuncs
{
  ...
  where TData : IDispatchDeadLetterData<TKey, TValue, TPayload>
  {
    ...
    try {
      ...
      if (message.Status != OutboxMessageStatus.DeadLettering)
        await services.UpdateIntegrationMessageAsync(message, message =>
          message.SetOutboxMessageStatus(OutboxMessageStatus.DeadLettering), ct);
      ...
      await services.UpdateIntegrationMessageAsync(message, message =>
        message.SetOutboxMessageStatus(OutboxMessageStatus.DeadLettered), ct);
